...
;; Clerk 解决了 `update-in` 在复杂嵌套与批量修改场景下的痛点, 再用 Specter 的 DSL(路径即程序)把同一类问题写到可读, 可组合, 可维护.
;;
...
;; Specter 的 `recursive-path` 定义一个可复用的递归导航器. 常见的两种遍历顺序:
...
;; 当数据不是普通的 map/vector(例如 Java 对象, deftype, 第三方库结构), 仍然可以通过 `defnav` 为它们定义 Specter 导航器.
;;
...
;; 可以用 `macroexpand-1` 直观看到这一点: `transform` 会把路径包装进 `com.rpl.specter/path`, 后者会做缓存与预编译.
...
;; 注意:
;;
;; - 如果用的是 `clojure -M:clerk`, 默认 classpath 没有 `criterium`, 本节会给出提示但不会报错.
